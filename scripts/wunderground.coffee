...
#
# Commands:
#   !weather <location> - short-term forecast
...
module.exports = (robot) ->
  robot.hear /^!weather (.*)$/i, (msg) ->
    ...
    get_data robot, msg, location, 'conditions', location.replace(/\s/g, '_'), send_current, 60*60*2
...
send_current = (msg, location, data) ->
  report = data.current_observation
  msg.send "Current weather for #{report.display_location.full}:"
  weather = "#{report.weather}, Temp: #{report.temp_f} F, Feels Like: #{report.feelslike_f} F, "
  if report.heat_index_f is not 'NA'
    weather += "Heat Index: #{report.heat_index_f} F, "
  else if report.windchill_f is not 'NA' 
    weather += "Wind Chill: #{report.windchill_f} F, "
    
  weather += "Humidity: #{report.relative_humidity}, Pressure: #{report.pressure_in}, Wind: #{report.wind_string}"
  msg.send weather

send_forecast = (msg, location, data) ->
  report = data.forecast.txt_forecast.forecastday[0]
  msg.send "Forecast for #{report.title}: #{report.fcttext}"
