...
class IrcBot extends Adapter
  ...
  names: (channel, nicks) ->
    console.log "Getting names for #{channel}"
    nickList = []
    nickList = (nick for nick, value of nicks)
    
    @robot.brain.data.channel or= {}
    @robot.brain.data.channel[channel] ?= {}
    @robot.brain.data.channel[channel].names = nickList
...
class IrcResponse extends Response
  ...
  action: (strings...) ->
    @robot.adapter.action @message.user, strings...
    
  command: (command, strings...) ->
    @robot.adapter.command command, strings...
    
  names: ->
    @robot.adapter.names @message.user.room
